...
def AUTO():
    ...
    if import_check.lower() == 'y' or import_check == "":
        ...
        if display_check.lower() == "y" or display_check == "":
            possiblefiles = subprocess.check_output("echo $(find /seti_data/raw_data -name *.zip)", shell=True).decode('utf-8')
    ...
    ###
    target = subprocess.check_output("echo $(ls $(pwd)/sdi/targets -t | head -n 1)", shell=True).decode('utf-8')
    target = target.replace("\n", "")
    radec = subprocess.check_output("echo $(ls $(pwd)/sdi/targets/%s -t -I raw_data | head -n 1)" % (target), shell=True).decode('utf-8')
    radec = radec.replace("\n", "")
    exposuretime = subprocess.check_output("echo $(ls $(pwd)/sdi/targets/%s/%s/air -t | head -n 1)" % (target, radec), shell=True).decode('utf-8')
    exposuretime = exposuretime.replace("\n", "")
    exposurepath = "$(pwd)/sdi/targets/target/air/radec/exposuretime"
    exposurepath = subprocess.check_output("echo $(pwd)/sdi/targets/%s/%s/air/%s" % (target, radec, exposuretime), shell=True).decode('utf-8')
